...
public class TeslaGunShootProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (main.getItem() != CreateCogsInvadersModItems.TESLA_GUN.get()) return;

		int energy = TeslaGunItem.getEnergy(main);
		...
		if (energy > 0) {
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, float damage, int knockback) {
							AbstractArrow entityToSpawn = new ElectricChargeEntity(
									CreateCogsInvadersModEntities.ELECTRIC_CHARGE.get(), level);
							...
							if (_shootFrom instanceof LivingEntity shooter) {
								...
								if (heldStack.is(CreateCogsInvadersModItems.TESLA_GUN.get())) {
			...
			if (main.getItem() instanceof TeslaGunItem)
			...
			TeslaGunItem.setEnergy(main, energy - 1);
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(main.getItem(), 10);
		...
		if (tieneBateria) {
			TeslaGunItem.setEnergy(main, TeslaGunItem.MAX_ENERGY);
